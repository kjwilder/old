...
class simplistiterator {
  class simplistnode {
   public:
    ...
    explicit simplistnode(
        const T& d, simplistnode* p = 0, simplistnode* n = 0) :
 ...
 public:
  explicit simplistiterator(simplistnode *x = 0) : p(x) { }
...
inline void simplist<T>::empty() {
  ...
  f = simplistiterator<T>(0);
  l = simplistiterator<T>(0);
  len = 0;
}
