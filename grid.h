...
class grid {
 ...
 public:
  ...
  grid() { initnew(0, 0); }
  explicit grid(int r) { initnew(r, 1); }
  grid(int r, int c) { initnew(r, c); }
  grid(const grid &m) : nr(0), nc(0), sto(0) { *this = m; }
  grid<T>& operator=(const grid &m);
  void subgrid(const grid& m, int r, int c, int numrows, int numcols) const;
  ...
  void set(int r, int c, T val) {
    if (inrange(r, c)) (*this)(r, c) = val;
  }
  T get(int r, int c) const {
    return (inrange(r, c) ? (*this)(r, c) : 0);
  }
...
template<class T>
void grid<T>::subgrid(
    const grid& m, int r, int c, int numrows, int numcols) const {
  assert(invariant() && m.invariant());
...
int grid<T>::loadpgm(const char* pgmname) {
  ...
  switch (mode) {
    ...
    case 5:
      for (j = 0; j < c; ++j) {
        for (i = 0; i < r; ++i) {
          ifs.read(static_cast<char *>(&((*this)(i, j))), 1);
        }
      }
      break;
    ...
    case 6:
      for (j=0; j < c; j++) {
        uchar tmpc[3];
        for (i = 0; i < r; ++i) {
          // ifs.read(static_cast<char*>(tmpc), 3);
          ifs.read(reinterpret_cast<char*>(tmpc), 3);
          (*this)(i, j) = static_cast<uchar>(0.212671 * tmpc[0] +
              0.715160 * tmpc[1] + 0.072169 * tmpc[2]);
        }
      }
      break;
